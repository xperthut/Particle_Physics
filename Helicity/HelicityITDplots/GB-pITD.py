...
Plotter.setYRange(0.0, 0.121);

Plotter.Plot.set_ylabel(r'$\mathit{\Delta \mathfrak{M}_g(\omega)}$', fontsize =14)
...
Plotter.Plot.yaxis.set_ticks(np.arange(0.0, 0.121, 0.02))
Plotter.Plot.yaxis.set_ticklabels([ 0.0, 0.020, 0.040, 0.060, 0.080, 0.10, 0.120], fontsize=10)
...
Plotter.Plot.fill_between(*(loadData("pITD_GB_band.txt")),
                          facecolor="g", alpha=0.55, linewidth=0)
...
p4 = Plotter.Plot.fill(0,0,'g', alpha=0.55, linewidth=0)
...
p44 = Plotter.plotLine(loadData("pITD_GB_line.txt"), color="g", linewidth=1.5, alpha=1.0, label=" legend");
#
...
Plotter.Plot.legend([(p4[-1], p44[0])], [ r' $\mathit{\Delta \mathfrak{M}_g(\omega)}$ (GBT)'], numpoints=1,prop={'size':12.0},loc= (0.55, 0.8))
